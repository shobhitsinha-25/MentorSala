...
export default function HeroSection() {
  return (
    <section id="hero" className="relative min-h-screen overflow-hidden bg-[#F8F9FE] px-4 pt-28 pb-16 lg:px-8">
      ...
      <div className="relative mx-auto max-w-7xl">
        {/* INLINE GRID ON ALL SCREENS: Left 5 Cols, Right 7 Cols */}
        <div className="grid grid-cols-12 items-center gap-2 sm:gap-6 lg:gap-8">
          
          {/* LEFT COLUMN: Hero Copy & Actions (5 Columns) */}
          <div className="col-span-5 flex flex-col items-start text-left">
            
            {/* Trusted Badge */}
            <div className="inline-flex items-center gap-1 sm:gap-2 rounded-full border border-purple-200 bg-white px-2 sm:px-3.5 py-1 shadow-sm">
              ...
              <span className="text-[10px] sm:text-xs font-semibold text-[#4F46E5] flex items-center gap-1">
            ...
            {/* Main Headline */}
            <h1 className="mt-3 sm:mt-6 text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black tracking-tight text-slate-900 leading-[1.1]">
              Every Rank <br />
              Has a <br />
            ...
            {/* Subtext */}
            <p className="mt-2 sm:mt-6 text-xs sm:text-base text-slate-600 leading-snug sm:leading-relaxed max-w-lg">
            ...
            {/* Primary Action CTAs */}
            <div className="mt-4 sm:mt-8 flex items-center gap-2 sm:gap-4 flex-wrap">
              <span className="text-[11px] sm:text-sm font-semibold text-slate-600">
                Start your journey with <span className="text-[#5B46F6] font-bold">MentorSala</span>
              </span>

              <Link
                to="/signup"
                className="group inline-flex items-center justify-center gap-1.5 sm:gap-2 rounded-xl bg-[#5B46F6] px-3.5 sm:px-7 py-2 sm:py-3.5 text-xs sm:text-base font-semibold text-white shadow-md shadow-indigo-500/25 transition-all duration-300 hover:bg-[#4C36E4] hover:shadow-indigo-500/40 hover:-translate-y-0.5 active:translate-y-0"
              >
                Start Free
                <ArrowRight size={14} className="transition-transform group-hover:translate-x-1 sm:w-[18px] sm:h-[18px]" />
              </Link>
            </div>

            {/* Feature Checklist */}
            <div className="mt-3 sm:mt-6 flex flex-wrap items-center gap-3 sm:gap-6 text-[10px] sm:text-xs font-medium text-slate-500">
            ...
            {/* Mini Stats Cards */}
            <div className="mt-5 sm:mt-10 grid w-full grid-cols-3 gap-1.5 sm:gap-3">
              <div className="rounded-xl sm:rounded-2xl border border-slate-100 bg-white p-2 sm:p-3.5 shadow-sm">
                <p className="text-xs sm:text-xl font-extrabold text-slate-900">3,000+</p>
                <p className="mt-0.5 text-[8px] sm:text-[11px] font-medium text-slate-500 leading-tight">Active Students</p>
              </div>
              <div className="rounded-xl sm:rounded-2xl border border-slate-100 bg-white p-2 sm:p-3.5 shadow-sm">
                <p className="text-xs sm:text-xl font-extrabold text-slate-900">10+</p>
                <p className="mt-0.5 text-[8px] sm:text-[11px] font-medium text-slate-500 leading-tight">IIT/NIT Mentors</p>
              </div>
              <div className="rounded-xl sm:rounded-2xl border border-slate-100 bg-white p-2 sm:p-3.5 shadow-sm">
                <p className="text-xs sm:text-xl font-extrabold text-slate-900">4.9/5</p>
                <p className="mt-0.5 text-[8px] sm:text-[11px] font-medium text-slate-500 leading-tight">Student Rating</p>
              </div>
            </div>

          </div>

          {/* RIGHT COLUMN: Interactive 3D Dashboard Visualizer (7 Columns) */}
          <div className="col-span-7 relative">
            <div className="relative w-full max-w-4xl mx-auto py-2 sm:py-10 px-0 sm:px-4 flex justify-center items-center select-none [perspective:1400px]">
              
              {/* 3D FLOATING SPHERE ORBS */}
              <div className="hidden sm:block absolute -top-6 -left-6 w-12 sm:w-16 h-12 sm:h-16 rounded-full bg-gradient-to-tr from-purple-600 to-indigo-400 shadow-[inset_-4px_-4px_12px_rgba(0,0,0,0.4),0_15px_25px_rgba(99,102,241,0.4)] animate-bounce duration-[3000ms] z-20" />
              <div className="hidden sm:block absolute -bottom-8 -right-6 w-14 sm:w-20 h-14 sm:h-20 rounded-xl rotate-45 bg-gradient-to-br from-indigo-500 via-purple-600 to-blue-600 shadow-[0_20px_35px_rgba(79,70,229,0.5)] z-20 animate-pulse" />

              {/* MAIN 3D TILT CONTAINER */}
              <div className="relative w-full rounded-[18px] sm:rounded-[36px] bg-gradient-to-tr from-[#6366F1] via-[#4F46E5] to-[#9333EA] p-1.5 sm:p-5 shadow-[0_20px_40px_-10px_rgba(79,70,229,0.35)] sm:shadow-[0_35px_60px_-15px_rgba(79,70,229,0.45)] transition-all duration-700 hover:scale-[1.01] [transform:rotateX(6deg)_rotateY(-4deg)_rotateZ(1deg)] sm:[transform:rotateX(8deg)_rotateY(-6deg)_rotateZ(1deg)] hover:[transform:rotateX(0deg)_rotateY(0deg)_rotateZ(0deg)]">
                
                {/* INNER WHITE DASHBOARD CANVAS */}
                <div className="relative overflow-hidden rounded-[14px] sm:rounded-[28px] bg-[#F8FAFC] p-2 sm:p-5 text-left shadow-inner border border-white/40">
                  
                  {/* HEADER BAR */}
                  <div className="flex items-center justify-between border-b border-slate-200/80 pb-1.5 sm:pb-3">
                    ...
                    <div className="flex items-center gap-1 sm:gap-3">
                      ...
                      <button className="relative rounded-xl p-1 sm:p-2 text-slate-400 hover:bg-slate-100 transition-colors">
                        <Bell size={12} className="sm:w-4 sm:h-4" />
                        <span className="absolute top-1 right-1 h-1.5 w-1.5 rounded-full bg-rose-500"></span>
                      </button>

                      <div className="flex items-center gap-1 sm:gap-2 rounded-full border border-slate-200 bg-white py-0.5 sm:py-1 px-1.5 sm:px-3 shadow-xs">
                        <img 
                          className="h-4 w-4 sm:h-7 sm:w-7 rounded-full object-cover ring-1 ring-indigo-50" 
                  ...
                  <div className="mt-2 sm:mt-4 grid grid-cols-12 gap-1.5 sm:gap-3.5">
                    
                    {/* LEFT SIDEBAR */}
                    <div className="col-span-4 lg:col-span-3 rounded-lg sm:rounded-2xl bg-white p-1.5 sm:p-3 shadow-xs border border-slate-100 flex flex-col justify-between gap-1 text-[8px] sm:text-[11px] font-semibold text-slate-500">
                      <div className="space-y-0.5 sm:space-y-1">
                        <div className="flex items-center gap-1 sm:gap-2.5 rounded-md sm:rounded-xl bg-indigo-600 px-1.5 sm:px-3 py-1 sm:py-2 text-white shadow-xs">
                          <LayoutDashboard size={10} className="sm:w-3.5 sm:h-3.5" />
                          <span className="truncate">Dashboard</span>
                        </div>
                        <div className="flex items-center gap-1 sm:gap-2.5 rounded-md sm:rounded-xl px-1.5 sm:px-3 py-1 sm:py-2 hover:bg-slate-50 transition-colors cursor-pointer">
                          <BookOpen size={10} className="sm:w-3.5 sm:h-3.5" />
                          <span className="truncate">Study Plan</span>
                        </div>
                        <div className="flex items-center gap-1 sm:gap-2.5 rounded-md sm:rounded-xl px-1.5 sm:px-3 py-1 sm:py-2 hover:bg-slate-50 transition-colors cursor-pointer">
                          <FileText size={10} className="sm:w-3.5 sm:h-3.5" />
                          <span className="truncate">Mock Tests</span>
                        </div>
                        <div className="flex items-center gap-1 sm:gap-2.5 rounded-md sm:rounded-xl px-1.5 sm:px-3 py-1 sm:py-2 hover:bg-slate-50 transition-colors cursor-pointer">
                          <Bot size={10} className="sm:w-3.5 sm:h-3.5" />
                          <span className="truncate">AI Mentor</span>
                        </div>
                        <div className="flex items-center gap-1 sm:gap-2.5 rounded-md sm:rounded-xl px-1.5 sm:px-3 py-1 sm:py-2 hover:bg-slate-50 transition-colors cursor-pointer">
                          <LineChart size={10} className="sm:w-3.5 sm:h-3.5" />
                          <span className="truncate">Performance</span>
                        </div>
                      </div>

                      {/* Study Streak Card */}
                      <div className="mt-1 sm:mt-2 rounded-md sm:rounded-xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-1 sm:p-3 text-white shadow-xs">
                        <p className="text-[7px] sm:text-[9px] font-bold text-slate-400 uppercase">Streak</p>
                        <div className="mt-0.5 flex items-baseline justify-between">
                          <div>
                            <span className="text-xs sm:text-2xl font-black">12</span>
                            <span className="text-[8px] sm:text-[10px] text-slate-300 ml-0.5">d</span>
                          </div>
                          <Flame size={10} className="text-orange-400 fill-orange-400 sm:w-4 sm:h-4" />
                        </div>
                      </div>
                    </div>

                    {/* CENTER & RIGHT CONTENT AREA */}
                    <div className="col-span-8 lg:col-span-9 space-y-1.5 sm:space-y-3.5">
                      
                      {/* TOP ROW: Study Plan & AI Rank Prediction */}
                      <div className="grid grid-cols-12 gap-1.5 sm:gap-3.5">
                        
                        {/* Today's Study Plan */}
                        <div className="col-span-12 sm:col-span-7 rounded-lg sm:rounded-2xl bg-white p-1.5 sm:p-3.5 shadow-xs border border-slate-100 flex flex-col justify-between">
                          <div className="flex items-center justify-between">
                            <h4 className="text-[9px] sm:text-xs font-bold text-slate-900">Study Plan</h4>
                            <span className="text-[7px] sm:text-[10px] font-bold text-indigo-600">View &rarr;</span>
                          </div>

                          <div className="mt-1 sm:mt-3 space-y-1 sm:space-y-2">
                            <div className="flex items-center justify-between rounded-md sm:rounded-xl bg-slate-50 p-1 sm:p-2.5 text-[8px] sm:text-[10px]">
                              <div>
                                <p className="font-bold text-slate-800">Physics</p>
                                <p className="text-slate-400 text-[7px] sm:text-[9px]">Rotational</p>
                              </div>
                            ...
                            <div className="flex items-center justify-between rounded-md sm:rounded-xl bg-slate-50 p-1 sm:p-2.5 text-[8px] sm:text-[10px]">
                              <div>
                                ...
                                <p className="text-slate-400 text-[7px] sm:text-[9px]">Organic</p>
                              </div>
                        ...
                        <div className="col-span-12 sm:col-span-5 rounded-lg sm:rounded-2xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-1.5 sm:p-3.5 text-white shadow-xs border border-indigo-500/20 flex flex-col justify-between">
                          <div>
                            <div className="flex items-center justify-between">
                              <span className="text-[7px] sm:text-[10px] font-bold text-indigo-300 uppercase">Predicted AIR</span>
                              <ExternalLink size={8} className="text-slate-400 sm:w-3 sm:h-3" />
                            </div>
                            <p className="mt-0.5 text-sm sm:text-3xl font-black text-white tracking-tight">2,458</p>
                            <span className="inline-block rounded-full bg-emerald-500/20 px-1 py-0.5 text-[7px] sm:text-[9px] font-bold text-emerald-400">Top 1.2%</span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-12 gap-1.5 sm:gap-3.5">
                        ...
                        <div className="col-span-6 rounded-lg sm:rounded-2xl bg-white p-1.5 sm:p-3.5 shadow-xs border border-slate-100 flex flex-col justify-between">
                          <span className="text-[8px] sm:text-[10px] font-bold text-slate-900">Next Session</span>
                          <div className="mt-1 flex items-center gap-1 sm:gap-3">
                            <img className="h-5 w-5 sm:h-10 sm:w-10 rounded-md sm:rounded-xl object-cover" src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=120&q=80" alt="Mentor" />
                            <div className="overflow-hidden">
                              <p className="text-[8px] sm:text-xs font-bold text-slate-800 truncate">Aditya P.</p>
                              <p className="text-[7px] sm:text-[9px] text-indigo-600 truncate">IIT Jammu</p>
                            </div>
                          </div>
                        </div>

                        {/* Test Performance Radial Gauge */}
                        <div className="col-span-6 rounded-lg sm:rounded-2xl bg-white p-1.5 sm:p-3.5 shadow-xs border border-slate-100 flex items-center justify-between">
                          <div>
                            <p className="text-[8px] sm:text-[10px] font-bold text-slate-900">Performance</p>
                            <p className="text-[7px] sm:text-[9px] text-slate-400 mt-0.5">Top Score</p>
                            <p className="text-[10px] sm:text-sm font-black text-slate-800">240/300</p>
                          </div>

                          <div className="relative flex h-8 w-8 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-indigo-50">
                            <span className="text-[8px] sm:text-xs font-black text-slate-900">78%</span>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between rounded-lg sm:rounded-2xl bg-gradient-to-r from-indigo-50 via-purple-50 to-indigo-50 p-1.5 sm:p-3 border border-indigo-100/80 shadow-xs">
                        <div className="flex items-center gap-1 sm:gap-3">
                          <div className="flex h-5 w-5 sm:h-8 sm:w-8 items-center justify-center rounded-md sm:rounded-xl bg-indigo-600 text-white">
                            <Bot size={10} className="sm:w-4 sm:h-4" />
                          </div>
                          <div>
                            <p className="text-[8px] sm:text-xs font-bold text-slate-900">AI Mentor</p>
                            <p className="text-[7px] sm:text-[10px] text-slate-500 truncate max-w-[80px] sm:max-w-none">How can I help today?</p>
                          </div>
                        </div>
                        <button className="rounded-md sm:rounded-xl bg-indigo-600 px-1.5 sm:px-3.5 py-1 text-[7px] sm:text-[10px] font-bold text-white shadow-xs">
                          Ask
                        </button>
                      </div>
